...
SELECT count(hire_date) as "employees hired after 2019" from employee where hire_date > "2019-01-10";

/*Q3. List the departments and the total salary of all employees in each department, ordered by the total salary.*/
SELECT department, sum(salary) as "total salary of all employees in the department" from employee
group by department
order by sum(salary);
...
SELECT first_name,last_name, max(salary) as "top 3 highest-paid" from employee
group by first_name,last_name
order by max(salary) desc limit 3;

/*Q6. Find the department with the minimum average salary.*/
SELECT department, avg(salary) as "Average Salary of employee" from employee
group by department
...
SELECT department, count(emp_id) as "total number of employees" from employee
group by department
order by count(emp_id);

/*Q8. Find the average salary of employees who were hired before 2020.*/
SELECT hire_date, avg(salary) as "average salary" from employee
group by hire_date
...
SELECT first_name,last_name, max(hire_date) as "employees in the IT department" from employee where department = "IT"
group by first_name,last_name
